...
const fs = require('fs'),
	...
	internals = ['securePath', 'serverRender', 'restfulApi', 'uploadStart', 'uploadEnd', 'socketOpen', 'socketClose'],
	start = { __proto__: null },
	stats = { __proto__: null },
	pmcbs = { __proto__: null },
	qcbs = { __proto__: null },
			...
			if (dataType(vars) === 'Object') {
				...
				if (Array.isArray(loop) && loop.length > 0) {
					...
					const apiname = op['!api'],
						api = apis[apiname];
					if (apiname === 'socketClose' && auth.internal) {
						if (auth.skid in qcbs) {
							const e = Error('socket_closed');
							for (const n in qcbs[auth.skid]) {
								clearTimeout(qcbs[auth.skid][n].tmo);
								qcbs[auth.skid][n].func(e);
							}
							delete qcbs[auth.skid];
						}
					}
...
//typeof data === number means close the socket
globalThis.pushMessage = (auth, data) => new Promise((resolve, reject) => {
	if (web) {
		web.write(++pmid + '\n' + toJsex({
			__proto__: null,
			type: 'push',
...
globalThis.askQuestion = (auth, data, timeout) => new Promise((resolve, reject) => {
	if (web) {
		const id = ++qid;
		web.write(id + '\n' + toJsex({
			__proto__: null,
		...
		ensurePath(qcbs, auth.skid)[id] = {
			func: resolve,
			tmo: setTimeout(() => {
				qcbs[auth.skid][id].func(Error('timed_out'));
				delete qcbs[auth.skid][id];
			}, timeout)
		};
	} else {
		resolve(Error('webserver_disconnected'));
	}
});
let web,
	pmid = 0,
...
if (process.stdin.isTTY) {
	...
	const statLog = m => `[${m}] ${stats[m].active} active, ${stats[m].done} done.\n`,
		startServer = err => {
			if (err) {
				...
				socket.on('error', function () {
					...
					for (let i = 0; i < data.length; ++i) {
						...
						if (i < data.length - 1) {
							...
							if (j === 2) {
								if (last[1] === 'push') {//push result
									if (last[0] in pmcbs) {
										pmcbs[last[0]](last[2]);
										delete pmcbs[last[0]];
									}
								} else if (typeof last[1] === 'number') {//anwser from client
									if (last[1] in qcbs && last[0] in qcbs[last[1]]) {
										clearTimeout(qcbs[last[1]][last[0]].tmo);
										qcbs[last[1]][last[0]].func(last[2]);
										delete qcbs[last[1]][last[0]];
									}
								} else {
